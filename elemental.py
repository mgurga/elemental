...
parser = argparse.ArgumentParser(
    description='give the elemental server some command line arguments')

parser.add_argument('--port', type=int,
                    help='what port the provider should run on', default=5005)
parser.add_argument('--host', type=str,
                    help='what host ip the provider should run on', default="")
parser.add_argument('--encoding', type=str,
                    help='what encoding algorithm scheme to use, client must be the same', default="ascii")
parser.add_argument('--providername', type=str,
                    help='name of the provider', default="LOCALPROVIDER")
parser.add_argument('--storagepath', type=str,
                    help='path of the provider storage', default="providerstorage")
parser.add_argument('--jsonfileindent', type=int,
                    help='how many indents to put in json files', default=2)
parser.add_argument('--sendotherusersdata', type=bool,
                    help='send everyone messages (EXPERIMENTAL)', default=False)
parser.add_argument('--providernameinconsole', type=str,
                    help='name of the provider in the console', default="[SERVER]")
...
class ECONFIG:
    ...
    # located in the base of providerstorage, must be 64x64
    providericonfilename = "icon.png"
...
def new_client(clientsocket, addr):
    ...
    while True:
        try:
            clientmsg = clientsocket.recv(16384)
            print("(" + getTime() + ") | " + ECONFIG.providernameinconsole + " <== [" + str(
                addr[0]) + ":" + str(addr[1]) + "] | " + clientmsg.decode(ECONFIG.encoding))
            resp = client.rawcommand(clientmsg.decode(ECONFIG.encoding))
            print("(" + getTime() + ") | " + ECONFIG.providernameinconsole +
                  " ==> [" + str(addr[0]) + ":" + str(addr[1]) + "] | " + resp)
            clientsocket.send(resp.encode(ECONFIG.encoding))
        except socket.error as e:
            print(
                "(" + getTime() + ") | closing connection because of socket error, details below")
            print(e)
            break
        except:
        ...
        for i in range(0, len(clients)):
            if not clients[i].usernm == user.usernm:
                if servername in clients[i].joinedservers:
                    ...
                    print("(" + getTime() + ") | " + ECONFIG.providernameinconsole +
                          " ==> [" + str(addr[0]) + ":" + str(addr[1]) + "] | " + json.dumps(jsontext))
                    clients[i].clientsocket.send(
                        jsontext.encode(ECONFIG.encoding))
...
print("running elemental server on " + str(ECONFIG.host) + ":" +
      str(ECONFIG.port) + " encoded with " + ECONFIG.encoding)
